  ...
  return (
    <div data-testid={testId} className="m-3 rounded-sm border border-solid border-(--hl-md) p-3">
      <div className="flex items-start gap-2">
        <div
          className={`inline-block shrink-0 rounded-sm px-2 py-[2px] text-center text-xs font-semibold text-white ${tag.className}`}
        >
          {tag.label}
        </div>
        <div className="min-w-0 flex-1">
          <div>
            ...
            {inlineResults && !isSkipped && results.length > 0 && (
              <span className="ml-2 text-xs whitespace-nowrap text-neutral-400">
                {`(${passedTests}/${results.length} tests passed)`}
              </span>
            )}
          </div>
          {!inlineResults && !isSkipped && stats && <div className="text-sm text-neutral-400">{stats}</div>}
